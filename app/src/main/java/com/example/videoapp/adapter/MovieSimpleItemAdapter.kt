...
import com.example.videoapp.databinding.ItemMoviesSimpleBinding
import com.example.videoapp.response.MoviesListResponse
import com.example.videoapp.utils.POSTER_BASE_URL
import javax.inject.Inject

class MovieSimpleItemAdapter @Inject() constructor(): PagingDataAdapter<MoviesListResponse.Result
        , MovieSimpleItemAdapter.ViewHolder>(differCallback) {

    private lateinit var binding: ItemMoviesSimpleBinding

    private lateinit var context: Context

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MovieSimpleItemAdapter.ViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        binding = ItemMoviesSimpleBinding.inflate(inflater, parent, false)
        context = parent.context
        return ViewHolder()
    }

    override fun onBindViewHolder(holder: MovieSimpleItemAdapter.ViewHolder, position: Int) {
